...
def Seq2Seq(output_dim, output_length, hidden_dim=None, depth=1, broadcast_state=True, inner_broadcast_state=True,
            teacher_force=False, peek=False, dropout=0., **kwargs):
    '''
    ...
    if not hidden_dim:
        hidden_dim = output_dim
    encoder = RecurrentContainer(readout=True, state_sync=inner_broadcast_state, input_length=shape[1], unroll=unroll,
                                 stateful=stateful, return_states=broadcast_state)
    ...
    for i in range(depth[1]):
        ...
        decoder.add(
            LSTMDecoderCell(output_dim=output_dim, hidden_dim=hidden_dim, batch_input_shape=(shape[0], output_dim),
                            **kwargs))
