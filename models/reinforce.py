...
class PolicyGradientREINFORCE(object):
    def __init__(self, session,
        ...
        self.session.run(tf.global_variables_initializer())  # tf.initialize_variables(var_lists))
    ...
    def create_variables(self):
        ...
        with tf.name_scope("model_inputs"):
            ...
            self.mask = tf.placeholder(tf.float32, (None, self.num_actions), name="mask")
        ...
        with tf.name_scope("predict_actions"):
            ...
            self.predicted_actions = tf.multinomial(tf.mul(self.action_scores, self.mask), 1)
    ...
    def sampleAction(self, states, mask):
        # TODO: use this code piece when tf.multinomial gets better
        # sample action from current policy
        actions = self.session.run(self.predicted_actions, {self.states: states, self.mask: mask})[0]
        return actions[0]
    ...
    def updateModel(self):
        ...
        rewards_std = np.std(self.all_rewards)
        if rewards_std != 0:
        ...
        for t in range(N - 1):
            ...
            # prepare inputs
            states = self.state_buffer[t].transpose() # [np.newaxis, :]
