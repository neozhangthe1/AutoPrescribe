...
def prototype_search_with_coverage_state():
    state = prototype_search_state()

    # for coverage
    state['maintain_coverage']=True
    # now we only use 1 dimension for the coverage
    state['coverage_dim']=1

    # for linguistic coverage, we direct operate on alignment probabilities only
    state['use_linguistic_coverage'] = True
    # added by Zhaopeng Tu, 2015-12-16
    state['use_fertility_model']=False
    state['max_fertility']=2
    # we define 2 types of accumulated operation, for each position
    ...
    state['coverage_accumulated_operation']="additive"

    state['use_recurrent_coverage'] = False
    state['use_recurrent_gating_coverage'] = False
    # inputs to recurrent coverage
    state['use_probability_for_recurrent_coverage'] = False
    state['use_input_annotations_for_recurrent_coverage'] = False
    state['use_decoding_state_for_recurrent_coverage'] = False
